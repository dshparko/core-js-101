...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((res, rej) => {
    const [yes, no] = ['Hooray!!! She said "Yes"!', 'Oh no, she said "No".'];
    const error = 'Wrong parameter is passed! Ask her again.';
    if (typeof isPositiveAnswer !== 'boolean') rej(new Error(error));
    res(isPositiveAnswer ? yes : no);
  });
}
